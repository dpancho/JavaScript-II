...
let firstNamesAllCaps = runners.map(names => {
...
let ticketPriceTotal = runners.reduce(function(total, donate){
...
// Problem 1
//donated more than $250
let largeDonations = runners.filter(runner => {
  return runner['donation'] > 250;  
});
console.log(largeDonations)
// Problem 2
//Make all Last names caps as well.
let lastNamesAllCaps = runners.map(names => {
  return names['last_name'].toUpperCase();
});
console.log(firstNamesAllCaps);
// Problem 3
//list of only emails
